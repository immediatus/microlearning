...
class Settings(BaseSettings):
    ...
    CREATE_TABLES_ON_STARTUP: bool = Field(default=True, description="Create tables on startup")
    ...
    DALLE_API_KEY: Optional[str] = Field(default=None, description="DALL-E API key (usually same as OpenAI)")
    MIDJOURNEY_API_KEY: Optional[str] = Field(default=None, description="Midjourney API key")
    STABLE_DIFFUSION_API_KEY: Optional[str] = Field(default=None, description="Stable Diffusion API key")
    LEONARDO_AI_API_KEY: Optional[str] = Field(default=None, description="Leonardo AI API key")
    ...
    GOOGLE_CLOUD_TTS_KEY: Optional[str] = Field(default=None, description="Google Cloud Text-to-Speech API key")
    ...
    STABLE_VIDEO_API_KEY: Optional[str] = Field(default=None, description="Stable Video Diffusion API key")
    
    # AI Services - Music Generation
    SUNO_API_KEY: Optional[str] = Field(default=None, description="Suno AI music generation API key")
    AIVA_API_KEY: Optional[str] = Field(default=None, description="AIVA music generation API key")
    SOUNDRAW_API_KEY: Optional[str] = Field(default=None, description="Soundraw API key")
    
    # AI Services - Avatar Generation
    DID_API_KEY: Optional[str] = Field(default=None, description="D-ID avatar API key")
    SYNTHESIA_API_KEY: Optional[str] = Field(default=None, description="Synthesia API key")
    ...
    S3_BUCKET_NAME: Optional[str] = Field(default=None, description="S3 bucket for video storage")
    ...
    # Content Generation
    MAX_CONCURRENT_GENERATIONS: int = Field(default=5, description="Max concurrent content generations")
    CONTENT_GENERATION_TIMEOUT: int = Field(default=600, description="Content generation timeout in seconds")
    
    # Video Settings
    MAX_VIDEO_DURATION: int = Field(default=120, description="Max video duration in seconds")
    VIDEO_RESOLUTION: str = Field(default="1080x1920", description="Video resolution (9:16 aspect ratio)")
    VIDEO_FPS: int = Field(default=30, description="Video frames per second")
    
    # Quiz Settings
    DEFAULT_QUIZ_TIME_LIMIT: int = Field(default=5000, description="Default quiz time limit in milliseconds")
    MAX_QUIZ_QUESTIONS: int = Field(default=5, description="Maximum quiz questions per video")
...
class AIServiceConfig:
    ...
    VOICE_SYNTHESIS_SERVICES = {
        ...
        "standard": {
            "service": "azure",
            "voices": {
                "12-15": "en-US-JennyNeural",
                "9-11": "en-US-AriaNeural"
            },
            "fallback": "google"
        },
        "budget": {
            "service": "google",
            "voices": {
                "12-15": "en-US-Wavenet-F",
                "9-11": "en-US-Wavenet-E"
            },
            "fallback": None
        }
    }
