...
class LearningVideo(Base):
    ...
    content_project_id = Column(
        GUID, ForeignKey("content_projects.id"), nullable=True, unique=True, index=True
    )
    ...
    learning_objectives = Column(JSONArray, nullable=False)
    ...
    age_groups = Column(JSONArray, nullable=False)  # ["12-15"]
    subject_area = Column(String(100), nullable=False)  # "Physics", "Chemistry", etc.
    curriculum_standards = Column(JSONArray, default=list)  # aligned standards
    ...
    content_project = relationship(
        "ContentProject", back_populates="published_video", uselist=False
    )
...
class ContentProject(Base):
    ...
    # Error handling
    errors = Column(JSONArray, default=list)
    retry_count = Column(Integer, default=0)
    last_error = Column(Text, nullable=True)

    # Relationships
    creator = relationship("Creator", back_populates="content_projects")
    published_video = relationship(
        "LearningVideo", back_populates="content_project", uselist=False
    )
...
class ContentTemplate(Base):
    ...
    # Template properties
    age_groups = Column(JSONArray, nullable=False)
    difficulty_levels = Column(JSONArray, default=list)  # supported difficulty levels
