...
class ContentCacheEntry(Base):
    ...
    content_type = Column(String(100), nullable=False)  # script, image_url, audio_url, etc.
...
class ContentCacheManager:
    ...
    def _create_cache_key(self, cache_type: CacheType, parameters: Dict[str, Any]) -> str:
    ...
    async def _get_exact_match(self, cache_type: CacheType, parameters: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            async with get_db() as db:
                cache_entry = await db.query(ContentCacheEntry).filter(
                ...
                if cache_entry:
                    ...
                    if self.redis_client:
                        ...
                        await self.redis_client.setex(
                            cache_key, 
                            ttl, 
                            json.dumps(cache_entry.content_data)
                        )
    ...
    async def _get_semantic_match(self, cache_type: CacheType, parameters: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            async with get_db() as db:
                ...
                similar_entries = await db.query(ContentCacheEntry).filter(
                    ContentCacheEntry.cache_type == cache_type,
                    ContentCacheEntry.is_active == True,
                    ContentCacheEntry.expires_at > datetime.utcnow(),
                    ContentCacheEntry.normalized_params["age_group"].astext == age_group
                ).limit(10).all()
                ...
                for entry in similar_entries:
                    stored_concept = entry.normalized_params.get("concept", entry.normalized_params.get("prompt", ""))
                    similarity = self._calculate_text_similarity(concept.lower(), stored_concept.lower())
                ...
                if best_match:
                    ...
                    logger.info(f"Semantic cache hit with {best_similarity:.2f} similarity")
    ...
    async def _get_fuzzy_match(self, cache_type: CacheType, parameters: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            async with get_db() as db:
                ...
                for entry in similar_entries:
                    if self._parameters_fuzzy_match(normalized_params, entry.normalized_params):
                        # Update stats
                        entry.hit_count += 1
    ...
    async def _get_template_match(self, cache_type: CacheType, parameters: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            async with get_db() as db:
                template_entries = await db.query(ContentCacheEntry).filter(
                    ContentCacheEntry.cache_type == cache_type,
                    ContentCacheEntry.is_active == True,
                    ContentCacheEntry.expires_at > datetime.utcnow()
                ).all()
                ...
                for entry in template_entries:
                    if self._template_parameters_match(template_params, entry.normalized_params):
                        # Update stats
                        entry.hit_count += 1
                        entry.last_accessed = datetime.utcnow()
                        await db.commit()
    ...
    def _parameters_fuzzy_match(self, params1: Dict[str, Any], params2: Dict[str, Any]) -> bool:
    ...
    def _template_parameters_match(self, template_params: Dict[str, Any], stored_params: Dict[str, Any]) -> bool:
    ...
    async def cleanup_expired_cache(self):
        ...
        try:
            async with get_db() as db:
                expired_count = await db.query(ContentCacheEntry).filter(
                    ContentCacheEntry.expires_at < datetime.utcnow()
                ).delete()
        ...
        try:
            async with get_db() as db:
                stats = {
                    ...
                    "active_entries": await db.query(ContentCacheEntry).filter(
                        ContentCacheEntry.is_active == True,
                        ContentCacheEntry.expires_at > datetime.utcnow()
                    ).count(),
                    "total_hits": await db.query(db.func.sum(ContentCacheEntry.hit_count)).scalar() or 0,
                    "cache_by_type": {}
                }
                
                for cache_type in CacheType:
                    type_count = await db.query(ContentCacheEntry).filter(
                        ContentCacheEntry.cache_type == cache_type,
                        ContentCacheEntry.is_active == True
                    ).count()
...
async def cache_script(concept: str, age_group: str, text: str, **kwargs) -> str:
    ...
    parameters = {"concept": concept, "age_group": age_group, **kwargs}
    content = {"text": text}
    return await content_cache.cache_content(CacheType.SCRIPT, parameters, content, **kwargs)
...
async def cache_image(prompt: str, image_url: str, **kwargs) -> str:
    ...
    return await content_cache.cache_content(CacheType.IMAGE, parameters, content, **kwargs)