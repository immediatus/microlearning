...
class CostTracker:
    ...
    SERVICE_RATES = {
        ...
        AIServiceType.ELEVENLABS_TTS: {"default": 0.18 / 1000},  # per character
        AIServiceType.RUNWAY_VIDEO: {"gen-2": 0.0125},  # per second
    }
    ...
    async def estimate_cost(
        self, service_type: AIServiceType, model: str, operation_params: dict[str, Any]
    ) -> Decimal:
        ...
        if service_type == AIServiceType.OPENAI_GPT:
            ...
            estimated_cost = Decimal(str(input_tokens)) * Decimal(
                str(model_rates["input"])
    ...
    async def check_budget_limits(
        self, creator_id: str, estimated_cost: Decimal
    ) -> dict[str, Any]:
        """Check if operation is within budget limits."""

        async with get_db() as db:
            budget = (
                await db.query(CreatorBudget)
                .filter(CreatorBudget.creator_id == creator_id)
                .first()
            )
            ...
            # Check limits
            within_limits = (
                new_daily <= budget.daily_limit
                and new_weekly <= budget.weekly_limit
                and new_monthly <= budget.monthly_limit
                and not budget.is_suspended
            )
    ...
    async def approve_request(
        ...
        async with get_db() as db:
            approval_request = (
                await db.query(ApprovalRequest)
                .filter(
            ...
            # Update cost entry
            cost_entry = (
                await db.query(AIServiceCost)
                .filter(AIServiceCost.id == approval_request.cost_entry_id)
                .first()
            )
    ...
    async def reject_request(
        self, approval_request_id: str, rejected_by: str, rejection_reason: str
    ) -> bool:
    ...
    async def record_actual_cost(
        self, cost_entry_id: str, actual_cost: Decimal, usage_metrics: dict[str, Any]
    ):
        ...
        async with get_db() as db:
            cost_entry = (
                await db.query(AIServiceCost)
                .filter(AIServiceCost.id == cost_entry_id)
                .first()
            )
            ...
            if cost_entry:
                ...
                cost_entry.characters_processed = usage_metrics.get(
                ...
                budget = (
                    await db.query(CreatorBudget)
                    .filter(CreatorBudget.creator_id == cost_entry.creator_id)
                    .first()
                )
    ...
    async def _reset_budget_periods_if_needed(self, budget: CreatorBudget, db):
        ...
        if now >= budget.daily_reset_at:
            ...
            budget.daily_reset_at = now.replace(
    ...
    def _create_operation_description(
        self, service_type: AIServiceType, model: str, params: dict[str, Any]
    ) -> str:
